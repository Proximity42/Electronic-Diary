...
from class_journal.models import Mark, Subject, ClassStudents, StudyClassSubjectsList, AssignedMark
...
MONTHS_IN_TERMS = {"1_term": [9, 10], "2_term": [11, 12], "3_term": [1, 2, 3], "4_term": [4, 5]}
...
def get_student_journal(request):
    ...
    study_class_subjects_lists = StudyClassSubjectsList.objects.filter(study_class=class_students.study_class)
    subjects = [study_class_subjects_list.subject for study_class_subjects_list in study_class_subjects_lists]
    assigned_marks = AssignedMark.objects.filter(student=student, subject__in=subjects).prefetch_related("mark")
    marks = [assigned_mark.mark for assigned_mark in assigned_marks]
...
def get_teacher_journal(request):
    ...
    if study_classes:
        student_class = get_students_class_by_indx(request, study_classes)
        ...
        context = {
            ...
            "begin_study_year": datetime.date.today().year,
            "end_study_year": datetime.date.today().year + 1
        }
    else:
        context = {}
    return context
